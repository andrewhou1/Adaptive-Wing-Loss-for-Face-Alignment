...
class HourglassModel():
	...
	def generate_data(self):
                HELEN_training_imgs = sorted(os.listdir('../training_data/HELEN/'))
                HELEN_training_imgs = ['../training_data/HELEN/'+i for i in HELEN_training_imgs]
                HELEN_Flip_training_imgs = sorted(os.listdir('../training_data/HELEN_Flip/'))
                HELEN_Flip_training_imgs = ['../training_data/HELEN_Flip/'+i for i in HELEN_Flip_training_imgs]
                LFPW_training_imgs = sorted(os.listdir('../training_data/LFPW/'))
                LFPW_training_imgs = ['../training_data/LFPW/'+i for i in LFPW_training_imgs]
                LFPW_Flip_training_imgs = sorted(os.listdir('../training_data/LFPW_Flip/')) 
                LFPW_Flip_training_imgs = ['../training_data/LFPW_Flip/'+i for i in LFPW_Flip_training_imgs]
                AFW_training_imgs = sorted(os.listdir('../training_data/AFW/'))
                AFW_training_imgs = ['../training_data/AFW/'+i for i in AFW_training_imgs]
                AFW_Flip_training_imgs = sorted(os.listdir('../training_data/AFW_Flip/'))
                AFW_Flip_training_imgs = ['../training_data/AFW_Flip/'+i for i in AFW_Flip_training_imgs]
                all_training_imgs = HELEN_training_imgs+HELEN_Flip_training_imgs+LFPW_training_imgs+LFPW_Flip_training_imgs+AFW_training_imgs+AFW_Flip_training_imgs
                
                HELEN_landmarks = scipy.io.loadmat('../training_data/HELEN_training_landmarks.mat')['landmarks']
                HELEN_Flip_landmarks = scipy.io.loadmat('../training_data/HELEN_Flip_training_landmarks.mat')['landmarks']
                LFPW_landmarks = scipy.io.loadmat('../training_data/LFPW_training_landmarks.mat')['landmarks']
                LFPW_Flip_landmarks = scipy.io.loadmat('../training_data/LFPW_Flip_training_landmarks.mat')['landmarks']
                AFW_landmarks = scipy.io.loadmat('../training_data/AFW_training_landmarks.mat')['landmarks']
                AFW_Flip_landmarks = scipy.io.loadmat('../training_data/AFW_Flip_training_landmarks.mat')['landmarks']
                all_training_landmarks = np.concatenate([HELEN_landmarks, HELEN_Flip_landmarks, LFPW_landmarks, LFPW_Flip_landmarks, AFW_landmarks, AFW_Flip_landmarks], axis=0)
                
                return all_training_imgs, all_training_landmarks
	...
	def _train(self, nEpochs = 10, epochSize = 1000, saveStep = 500, validIter = 10):
		...
		with tf.name_scope('Train'):
			...
			#self.valid_gen = self.dataset._aux_generator(self.batchSize, self.nStack, normalize = True, sample_set = 'valid')
                        (all_training_imgs, all_training_landmarks) = self.generate_data()
                        startTime = time.time()
                        self.resume = {}
			#self.resume['accur'] = []
                        self.resume['loss'] = []
			#self.resume['err'] = []
                        for epoch in range(nEpochs):
                                permutation = np.random.permutation(len(all_training_imgs))
				...
				# Training Set
					...
					#img_train, gt_train, weight_train = next(self.generator)
                                        img_train = np.zeros((self.batchSize, 256, 256, 3))
                                        gt_train = np.zeros((self.batchSize, self.nStack, 64, 64, self.outDim))
                                        batch_idxes = permutation[(i*self.batchSize):((i+1)*self.batchSize)]
                                        relevant_landmarks = all_training_landmarks[batch_idxes]
                                        for j in range(self.batchSize):
                                                img_train[j] = cv2.imread(all_training_imgs[batch_idxes[j]])/127.5-1
                                                heatmap = self.generate_heatmap(relevant_landmarks[j])
                                                for k in range(self.nStack):
                                                        gt_train[j, k, :, :, :] = heatmap
                                        if i % saveStep == 0:
                                                if self.w_loss:
