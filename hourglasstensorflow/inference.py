...
class HourglassModel():
	...
	"""
	def __init__(self, nFeat = 512, nStack = 4, nModules = 1, nLow = 4, outputDim = 68, batch_size = 16, drop_rate = 0.2, lear_rate = 2.5e-4, decay = 0.96, decay_step = 2000, training = True, w_summary = True, tiny = True, attention = False,modif = True,w_loss = False, name = 'tiny_hourglass'):
		""" Initializer
		Args:
	...
	def generate_model(self):
		...
		with tf.device(self.gpu):
			...
			with tf.name_scope('loss'):
				...
				self.loss = self.adaptive_wing_loss()
			lossTime = time.time()	
		...
		del endTime, startTime, initTime, minimTime, lossTime, graphTime, inputTime
	...
	def generate_data(self):
                HELEN_testing_imgs = sorted(os.listdir('../testing_data/HELEN/'))
                HELEN_testing_imgs = ['../testing_data/HELEN/'+i for i in HELEN_testing_imgs]
                LFPW_testing_imgs = sorted(os.listdir('../testing_data/LFPW/'))
                LFPW_testing_imgs = ['../testing_data/LFPW/'+i for i in LFPW_testing_imgs]
                IBUG_testing_imgs = sorted(os.listdir('../testing_data/IBUG/'))
                IBUG_testing_imgs = ['../testing_data/IBUG/'+i for i in IBUG_testing_imgs]
                #all_testing_imgs = HELEN_testing_imgs+LFPW_testing_imgs+IBUG_testing_imgs
                #all_testing_imgs = HELEN_testing_imgs+LFPW_testing_imgs
                all_testing_imgs = IBUG_testing_imgs
                
                HELEN_landmarks = scipy.io.loadmat('../testing_data/HELEN_testing_landmarks.mat')['landmarks']
                LFPW_landmarks = scipy.io.loadmat('../testing_data/LFPW_testing_landmarks.mat')['landmarks']
                IBUG_landmarks = scipy.io.loadmat('../testing_data/IBUG_testing_landmarks.mat')['landmarks']
                #all_testing_landmarks = np.concatenate([HELEN_landmarks, LFPW_landmarks, IBUG_landmarks], axis=0)
                #all_testing_landmarks = np.concatenate([HELEN_landmarks, LFPW_landmarks], axis=0)
                all_testing_landmarks = IBUG_landmarks
                
                return all_testing_imgs, all_testing_landmarks

	def generate_heatmap(self, landmarks):
                heatmap = np.zeros((64, 64, 68))
                for i in range(68):
                        Gauss_map = np.zeros((64, 64))
                        mask_x = numpy.matlib.repmat(landmarks[i, 0]/4.0, 64, 64)
                        mask_y = numpy.matlib.repmat(landmarks[i, 1]/4.0, 64, 64)
                        x1 = np.arange(64)
                        x_map = numpy.matlib.repmat(x1, 64, 1)
                        y1 = np.arange(64)
                        y_map = numpy.matlib.repmat(y1, 64, 1)
                        y_map = np.transpose(y_map)
                        Gauss_map = np.sqrt((x_map-mask_x)**2+(y_map-mask_y)**2)
                        Gauss_map = np.exp(-0.5*Gauss_map/1)
                        heatmap[:, :, i] = Gauss_map
                return heatmap

	def compute_NME(self, heatmap, gt_landmarks):
                NME = 0
                for i in range(68):
                        curr_heatmap = np.reshape(heatmap[:, 3, :, :, i], (64, 64))
                        (predicted_y, predicted_x) = np.where(curr_heatmap == np.max(curr_heatmap))
                        predicted_y = predicted_y[0]
                        predicted_x = predicted_x[0]
                        true_y = (gt_landmarks[i, 1]-1)/4.0
                        true_x = (gt_landmarks[i, 0]-1)/4.0
                        curr_err = math.sqrt((predicted_y-true_y)**2+(predicted_x-true_x)**2)
                        NME = NME+curr_err
                NME = NME/68.0
                right_pupil_y = (gt_landmarks[36, 1]+gt_landmarks[37, 1]+gt_landmarks[38, 1]+gt_landmarks[39, 1]+gt_landmarks[40, 1]+gt_landmarks[41, 1])/4.0/6.0
                right_pupil_x = (gt_landmarks[36, 0]+gt_landmarks[37, 0]+gt_landmarks[38, 0]+gt_landmarks[39, 0]+gt_landmarks[40, 0]+gt_landmarks[41, 0])/4.0/6.0
                left_pupil_y = (gt_landmarks[42, 1]+gt_landmarks[43, 1]+gt_landmarks[44, 1]+gt_landmarks[45, 1]+gt_landmarks[46, 1]+gt_landmarks[47, 1])/4.0/6.0
                left_pupil_x = (gt_landmarks[42, 0]+gt_landmarks[43, 0]+gt_landmarks[44, 0]+gt_landmarks[45, 0]+gt_landmarks[46, 0]+gt_landmarks[47, 0])/4.0/6.0
                interpupil_dist = math.sqrt((right_pupil_y-left_pupil_y)**2+(right_pupil_x-left_pupil_x)**2)
                NME = NME/interpupil_dist
                return NME
	...
	def _test(self):
		...
		with tf.name_scope('Test'):
			...
			#self.valid_gen = self.dataset._aux_generator(self.batchSize, self.nStack, normalize = True, sample_set = 'valid')
                        (all_testing_imgs, all_testing_landmarks) = self.generate_data()
                        startTime = time.time()
                        self.resume = {}
			#self.resume['accur'] = []
                        self.resume['loss'] = []
			#self.resume['err'] = []
                        for epoch in range(1):
                                ...
                                avg_cost = 0.
                                cost = 0.
                                print('Epoch :' + str(epoch) + '/' + str(1) + '\n')
                                NME = 0
				# Training Set
                                for i in range(len(all_testing_imgs)):
					# DISPLAY PROGRESS BAR
					# TODO : Customize Progress Bar
                                        percent = ((i+1)/len(all_testing_imgs)) * 100
                                        num = np.int(20*percent/100)
                                        tToEpoch = int((time.time() - epochstartTime) * (100 - percent)/(percent))
                                        sys.stdout.write('\r Test: {0}>'.format("="*num) + "{0}>".format(" "*(20-num)) + '||' + str(percent)[:4] + '%' + ' -cost: ' + str(cost)[:6] + ' -avg_loss: ' + str(avg_cost)[:5] + ' -timeToEnd: ' + str(tToEpoch) + ' sec.')
                                        sys.stdout.flush()
					#img_train, gt_train, weight_train = next(self.generator)
                                        img_test = np.zeros((self.batchSize, 256, 256, 3))
                                        gt_test = np.zeros((self.batchSize, self.nStack, 64, 64, self.outDim))
                                    
                                        
                                        img_test[0] = cv2.imread(all_testing_imgs[i])/127.5-1
                                        heatmap = self.generate_heatmap(all_testing_landmarks[i])
                                        for k in range(self.nStack):
                                                gt_test[:, k, :, :, :] = heatmap
                                        if self.w_loss:
                                                c, out = self.Session.run([self.loss, self.output], feed_dict = {self.img : img_test, self.gtMaps: gt_test, self.weights: weight_train})
                                        else:
                                                c, out = self.Session.run([self.loss, self.output], feed_dict = {self.img : img_test, self.gtMaps: gt_test})
                                        #print(np.shape(self.output))
                                        #print(np.shape(self.gtMaps))
                                        NME = NME+self.compute_NME(out, all_testing_landmarks[i, :, :])
                                        cost += c
                                        avg_cost += c/len(all_testing_imgs)
                                        output_mat = {}
                                        output_mat['predicted_heatmaps'] = out
                                        name_parts = all_testing_imgs[i].split('/')
                                        scipy.io.savemat('predicted_heatmaps_adaptive_wing_loss/'+name_parts[-1]+'.mat', output_mat)
                                epochfinishTime = time.time()
                                avg_NME = NME/len(all_testing_imgs)
                                print('\n')
                                print('Avg NME: '+str(100*avg_NME))
	...
	def testing_init(self, nEpochs = 10, epochSize = 1000, saveStep = 500, dataset = None, load = None):
	...
	def adaptive_wing_loss(self):
                with tf.name_scope('adaptive_wing_loss'):
                     x = self.output - self.gtMaps
                     theta_over_epsilon_tensor = tf.fill(tf.shape(self.gtMaps), self.theta/self.epsilon)
                     A = self.omega*(1/(1+tf.pow(theta_over_epsilon_tensor, self.alpha-self.gtMaps)))*(self.alpha-self.gtMaps)*tf.pow(theta_over_epsilon_tensor, self.alpha-self.gtMaps-1)*(1/self.epsilon)
                     C = self.theta*A-self.omega*tf.log(1+tf.pow(theta_over_epsilon_tensor, self.alpha-self.gtMaps))
                     absolute_x = tf.abs(x)
                     losses = tf.where(tf.greater(self.theta, absolute_x), self.omega*tf.log(1+tf.pow(absolute_x/self.epsilon, self.alpha-self.gtMaps)), A*absolute_x-C)
